...
const ProfilePage = () => {
  ...
  const fetchblogbyuserid = async()=>{
    try{
      const response = await axios.get(`${API_PATH}/blogs/getblogbyuserid/${id}`)
  ...
  const deleteBlogById = async (blogId) => {
    ...
    try {
        const response = await axios.delete(`${API_PATH}/blogs/deleteblogbyid/${blogId}`);
          ...
          {blogs.length>0?(
            blogs.map((blog,index)=>(
              <div key={index} className="blog-item">
                ...
                }} className='blogdeletebtn'>Delete</button>
